...
int main() {
    ...
    Vertex *v1 = cringe.Name.find("A")->second;
...
/*
  ...
  // vertex.push_back(vv);
   edge = v1->getAdj();
   Graph sus = cringe.SubGraphCreate(cringe.graph,edge,vertex);
    std::cout <<    sus.vertexSet.size() << '\n';
    std::cout <<    sus.edgeSet.size() << '\n';

    cringe.unresolved_lines(cringe.graph,v1,vv,edge,vertex);
