...
public:
    ...
    */
    ...
    @brief Removes a vertex from the graph along with its incident edges.
    This function removes a given vertex from the graph, along with its incident edges.
    It updates the internal data structures of the graph, including the adjacency lists of vertices and the edge set.
    This function also updates the edge vector passed as a parameter to remove the edges incident to the removed vertex from it.
    @param v Pointer to the vertex to be removed.
    @param edge Reference to a vector of pointers to edges, where the edges incident to the removed vertex will be removed from.
...
protected:
    ...
    /**
     * @brief Adds a bidirectional edge between two vertices in the graph, using an existing edge as a template.
     * @param v1 Pointer to the first vertex.
     * @param v2 Pointer to the second vertex.
     * @param e Pointer to the existing edge to be used as a template.
     * @return True if the edge was added successfully, false otherwise.
     */
    ...
    int** distMatrix; /**< The distance matrix of the graph. */
    int** pathMatrix; /**< The path matrix of the graph. */
};
